...
首頁 `/` 列出即時去中心化交易所流動池：交易對、鏈、價格與深度取自鏈上儲備（已接線的原生鏈場地亦一併讀取）。可按鏈篩選或搜尋。分頁及篩選留在本瀏覽器工作階段。報價為即時價，並非成交保證。
...
`/pair/:chainId/:tokenA/:tokenB` 顯示定向價格、場地深度及近期兌換。YSK Mint 並不執行兌換。若要交易，請從交易對頁開啟該去中心化交易所。
