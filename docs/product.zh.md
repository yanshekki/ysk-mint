...
- **鏈** — 開關市場及持倉掃描所用的鏈，並有主流／非主流／全部開啟／全部關閉預設。**預設為主流**（ETH、AVAX、Base、Arb、ADA、NEAR、BNB、SOL、TRX、POL、SUI、TON、APT、OP，以及主流借貸／流動倉／質押組）。同一分頁有 DeFi 掃描組（借貸、流動倉、質押 × 主流／非主流），持倉會跳過未選的協議 RPC。RPC 策略（官方／PublicNode／1RPC／dRPC）及每鏈端點，包括僅存於本瀏覽器的自訂網址。關閉即跳過，不會畫假的 0。還原預設亦回到主流。
