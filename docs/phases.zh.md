...
- **靜態託管** — hashed `/assets` 可長駐 edge；HTML 與 `/version.json` 再驗證。頁腳烘焙 `v1.4.0`。Cloudflare 須 Bypass `/koios*`。建置時可用 `VITE_GA_MEASUREMENT_ID` 注入該站分析標籤；倉庫不存放即時編號。
