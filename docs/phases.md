...
- **Static hosting** — hashed `/assets` may stay at the edge; HTML and `/version.json` revalidate. The footer bakes `v1.4.2`. Cloudflare must Bypass `/koios*`. A site-specific analytics tag may be injected at build with `VITE_GA_MEASUREMENT_ID`; the repository does not store a live id.
